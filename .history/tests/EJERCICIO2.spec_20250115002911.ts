...
test.describe('Mercadolibre Ejercicio 2', () => {
  ...
  const filtros = {
    ...
    precioMaximo: process.env.npm_config_PRECIOMAX || process.env.PRECIOMAX || '3.000.000', // Valor por defecto 5000000
  };
  ...
  test('Autos Usados', async ({  }) => {
    ...
    const rangoPreciosXPath = `//div[contains(text(),"$${filtros.precioMinimo} a $${filtros.precioMaximo}")]`;

    // Validar el rango de precios en los resultados
    await expect(page.locator(rangoPreciosXPath)).toContainText(`$${filtros.precioMinimo} a $${filtros.precioMaximo}`);
