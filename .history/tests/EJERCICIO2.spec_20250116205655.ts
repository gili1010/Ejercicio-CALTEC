...
test.describe('Mercadolibre Ejercicio 2', () => {
  ...
  test('Autos Usados', async ({  }) => {
    ...
    try{
      ...
      const rangoKmXPath = `//div[contains(text(),"$${kmMinimoFormateado} a $${kmMaximoFormateado}")]`;

      // Validar el rango de precios en los resultados
      await expect(page.locator(rangoKmXPath)).toContainText(`$${kmMinimoFormateado} a $${kmMaximoFormateado}`);
